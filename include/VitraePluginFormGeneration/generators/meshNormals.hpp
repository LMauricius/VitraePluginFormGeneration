...
{
    inline Vitrae::StableMap<Vitrae::StringId, Vitrae::SharedSubBufferVariantPtr> generateMeshNormals(Vitrae::ComponentRoot &root, Vitrae::Mesh &mesh)
    {
        using namespace Vitrae;

        // ensure the dependency components are loaded
        mesh.prepareComponents(ParamList{VitraePluginFormGeneration::StandardParam::position});
    ...
    inline void setupMeshNormalGenerator(Vitrae::ComponentRoot &root)
    {
        ...
        meshGenerators.registerGeneratorForComponents({{Vitrae::StandardParam::normal.name}}, generateMeshNormals);
    }
}