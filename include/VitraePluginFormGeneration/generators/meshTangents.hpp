...
#include "VitraePluginFormGeneration/Standard/Params.hpp"
...
{
    inline Vitrae::StableMap<Vitrae::StringId, Vitrae::SharedSubBufferVariantPtr> generateMeshTangents(Vitrae::ComponentRoot &root, Vitrae::Mesh &mesh)
    {
        using namespace Vitrae;

        // ensure the dependency components are loaded
        mesh.prepareComponents(ParamList{VitraePluginFormGeneration::StandardParam::position, VitraePluginFormGeneration::StandardParam::normal,
            VitraePluginFormGeneration::StandardParam::coord_base});

        // Extract existing data
        std::span<const Triangle> triangles = mesh.getTriangles();
        StridedSpan<const glm::vec3>
            positions = mesh.getVertexComponentData<glm::vec3>(VitraePluginFormGeneration::StandardParam::position.name);
        StridedSpan<const glm::vec3>
            normals = mesh.getVertexComponentData<glm::vec3>(VitraePluginFormGeneration::StandardParam::normal.name);
        StridedSpan<const glm::vec3> texcoords =
            mesh.getVertexComponentData<glm::vec3>(VitraePluginFormGeneration::StandardParam::coord_base.name);
    ...
    inline void setupMeshTangentGenerator(Vitrae::ComponentRoot &root)
    {
        Vitrae::MeshGeneratorCollection &meshGenerators = root.getComponent<Vitrae::MeshGeneratorCollection>();
        Vitrae::Renderer &renderer = root.getComponent<Vitrae::Renderer>();
